...
{
	clear                                  # Limpa a tela
	echo -e "\033[01;31m"                  # Muda cor do texto para vermelho
	echo "
...
| |___| |_| | |_) |  __/ |  ____) |  __/ (__| |_| | |  | | |_| |_| | |_| | |_| | |_| | |_| |
 \_____\__, |_.__/ \___|_| |_____/ \___|\___|\__,_|_|  |_|\__|\__, |\___/ \___/ \___/ \___/ 
	...
	echo -e "\033[00;00m"             # Reseta cor do texto
}
...
apres   # Chama a função de apresentação logo no início

# Verifica se o usuário passou exatamente 3 argumentos
# Ex: ./script.sh 37.59.174.225 224 239
if [ $# -ne 3 ]
then
	echo "Usage: $0 <IP> <Range> (ex: 37.59.174.225 224 239)"

else

	# Valida se o range informado está entre 0 e 255
	# e se o início do range é menor ou igual ao fim
	if [[ "$2" -gt 255 || "$2" -lt 0 ]] || [[ "$3" -gt 255 || "$3" -lt 0 ]] || [ "$2" -gt "$3" ]
	then
		echo "\033[01;31m --- Erro no range --- \033[00;00m"
		exit 1
	else
	
		echo -e "\n\033[01;33m# Testando IP -> $1\n\033[00;00m"

		# Pega apenas os 3 primeiros octetos do IP base
		# Ex: 37.59.174.225 -> vira 37.59.174
		ip=$(echo "$1" | cut -d "." -f 1-3 2>/dev/null)

		# Faz um loop de IPs completando o último octeto
		# Ex: 37.59.174.224 até 37.59.174.239
		for i in $(seq $2 $3)
		do
